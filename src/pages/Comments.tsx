...
const Comments = () => {
  ...
  const addComment = async () => {
    ...
    try {
      ...
      setComments((prev) =>
        prev.map((comment) => (comment.id === localId ? { ...comment, id: commentFromAPI.id } : comment))
      );
    } catch (error) {
  ...
  const updateComment = async (id: number, body: string) => {
    if (id > 30) {
      alert("Data ini hanya tersimpan di frontend dan tidak bisa diupdate di API.");
      return;
    }
    ...
    setIsLoading(true);
    try {
      const updatedComment = await updateData<Comment>("comments", id, { body: updatedBody });
      setComments((prev) =>
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-gray-100 rounded-lg shadow-md mt-6">
      ...
      <div className="mb-4">
        <button onClick={addComment} className="w-full bg-blue-500 text-white py-2 rounded" disabled={isLoading}>
          {isLoading ? "Adding..." : "Add Comment"}
        </button>
      </div>
      <div className="space-y-3">
        {comments.map((comment) => (
          <div key={comment.id} className="flex flex-col bg-white p-3 rounded shadow">
            <p className="text-gray-700">{comment.body}</p>
            <div className="flex justify-end space-x-2 mt-2">
              <button onClick={() => updateComment(comment.id, comment.body)} className="text-yellow-500">✎</button>
